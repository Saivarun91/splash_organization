...
export function WorkflowTab({ project }) {
    ...
    const handleRequestSuggestions = async (description, targetAudience = null, campaignSeason = null) => {
        ...
        try {
            ...
            const response = await organizationAPI.updateCollectionDescription(
                project.id,
                description || "",
                null,
                targetAudience,
    ...
    useEffect(() => {
        ...
        fetchCollectionData()
    }, [project?.collection?.id, project?.collection_id])
            ...
            switch (activeStep) {
                ...
                case 2:
                    ...
                    if (project?.id && collectionData?.id) {
                        const response = await organizationAPI.updateCollectionSelections(
                            project.id,
                            collectionData.id,
                            currentSelections,
                            uploadedImages
                ...
                case 3:
                    ...
                    if (selectedModel && collectionData?.id) {
                        try {
                            setLoading(true)
                            const response = await organizationAPI.selectModel(
                                collectionData.id,
                                selectedModel.type,
                                selectedModel
                            )
