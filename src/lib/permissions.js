...
export function hasProjectRole(project, user, requiredRole = 'viewer') {
    ...
    // Check if user is organization owner (they get owner access to all projects)
...
export function canEditProject(project, user) {
    // Organization owners can edit all projects
    if (user?.organization_role === 'owner') {
        return true
    }
...
export function isProjectOwner(project, user) {
    // Organization owners are treated as project owners
    if (user?.organization_role === 'owner') {
        return true
    }
