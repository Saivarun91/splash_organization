...
export default function CreditsUsagePage() {
            ...
            <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
                <div className="bg-white rounded-lg shadow-xl w-full max-w-5xl max-h-[90vh] overflow-hidden flex flex-col">
                    <div className="flex items-center justify-between p-6 border-b border-gray-200">
                        <div>
                            <h2 className="text-2xl font-bold text-gray-900">{t("orgPortal.creditUsageHistory") || "Credit Usage History"}</h2>
                    ...
                    <div className="p-6 border-b border-gray-200 bg-gray-50">
                        <div className="flex items-center gap-4">
                            <div className="flex items-center gap-2">
                                ...
                                <span className="text-sm font-medium text-gray-700">{t("orgPortal.filterByDate") || "Filter by date"}:</span>
                            </div>
                            <input
                                type="date"
                            ...
                            />
                            <span className="text-gray-500">{t("orgPortal.to") || "to"}</span>
                            <input
                                type="date"
                    ...
                    <div className="flex-1 overflow-y-auto p-6">
                        {logs.length > 0 ? (
                                ...
                                <table className="w-full">
                                    <thead>
                                        <tr className="border-b border-gray-200 bg-gray-50">
                                            <th className="text-left py-3 px-4 font-semibold text-gray-900">{t("orgPortal.date") || "Date"}</th>
                                            <th className="text-left py-3 px-4 font-semibold text-gray-900">{t("orgPortal.type") || "Type"}</th>
                                            <th className="text-left py-3 px-4 font-semibold text-gray-900">{t("orgPortal.amount") || "Amount"}</th>
                                            <th className="text-left py-3 px-4 font-semibold text-gray-900">
                                                {t("orgPortal.balanceAfter") || "Balance After"}
                                            </th>
                                            <th className="text-left py-3 px-4 font-semibold text-gray-900">{t("orgPortal.reason") || "Reason"}</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        {logs.map((log) => (
                            ...
                            <div className="text-center py-12">
                                <p className="text-gray-600">{t("orgPortal.noCreditLogsFound") || "No credit logs found"}</p>
                            </div>
                        )}
                    </div>
                </div>
        ...
        return (
            <div className="p-8 flex items-center justify-center min-h-screen">
                <div className="text-center">
                    ...
                    <p className="text-gray-600">{t("orgPortal.loadingCreditHistory") || "Loading credit history..."}</p>
                </div>
            </div>
        );
    }
    ...
    return (
        <div className="p-8">
            <div className="mb-8">
                <h1 className="text-3xl font-bold text-gray-900 mb-2">{t("orgPortal.creditsUsageHistory") || "Credits Usage History"}</h1>
                <p className="text-gray-600">{t("orgPortal.viewIndividualCreditUsage") || "View individual credit usage by organization members"}</p>
            </div>

            {creditUsage && creditUsage.summary && (
                <div className="mb-6 bg-white rounded-lg shadow-md p-6 border border-gray-200">
                    <h2 className="text-lg font-semibold text-gray-900 mb-4">{t("orgPortal.summary") || "Summary"}</h2>
                    <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                        <div>
                            <p className="text-sm text-gray-600">{t("orgPortal.totalDebits") || "Total Debits"}</p>
                            <p className="text-2xl font-bold text-red-600">{creditUsage.summary.total_debits || 0}</p>
                        </div>
                        <div>
                            <p className="text-sm text-gray-600">{t("orgPortal.totalCredits") || "Total Credits"}</p>
                            <p className="text-2xl font-bold text-green-600">
                                {creditUsage.summary.total_credits || 0}
                            </p>
                        </div>
                        <div>
                            <p className="text-sm text-gray-600">{t("orgPortal.netUsage") || "Net Usage"}</p>
                            <p className="text-2xl font-bold text-gray-900">{creditUsage.summary.net_usage || 0}</p>
                        </div>
                        <div>
                            <p className="text-sm text-gray-600">{t("orgPortal.currentBalance") || "Current Balance"}</p>
                ...
                <div className="overflow-x-auto">
                    <table className="w-full">
                        <thead className="bg-gray-50 border-b border-gray-200">
                            <tr>
                                <th className="text-left py-4 px-6 font-semibold text-gray-900">{t("orgPortal.user") || "User"}</th>
                                <th className="text-left py-4 px-6 font-semibold text-gray-900">{t("orgPortal.email") || "Email"}</th>
                                <th className="text-left py-4 px-6 font-semibold text-gray-900">{t("orgPortal.role") || "Role"}</th>
                                <th className="text-left py-4 px-6 font-semibold text-gray-900">{t("orgPortal.actions") || "Actions"}</th>
                            </tr>
                        </thead>
                        <tbody>
                            {membersLoading ? (
                                ...
                                <tr>
                                    <td colSpan={4} className="text-center py-12">
                                        ...
                                        <p className="text-gray-600">{t("orgPortal.noUsersFound") || "No users found"}</p>
                                    </td>
                                </tr>
                            )}
                        </tbody>
